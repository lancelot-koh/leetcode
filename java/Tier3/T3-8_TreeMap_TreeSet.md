...
Integer lo2 = map.lowerKey(x);     // largest key < x
Integer hi2 = map.higherKey(x);    // smallest key > x
...
public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
    ...
    for (int i = 0; i < nums.length; i++) {
        ...
        window.add(num);
        if (i >= k) window.remove((long) nums[i - k]);  // maintain size-k window
    }
    return false;
}
```
...
public List<List<Integer>> getSkyline(int[][] buildings) {
    ...
    for (int[] e : events) {
        if (e[1] < 0)  heights.merge(-e[1], 1, Integer::sum);   // building starts
        else {
            heights.merge(e[1], -1, Integer::sum);               // building ends
